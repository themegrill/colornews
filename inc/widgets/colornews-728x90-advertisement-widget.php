...
class colornews_728x90_advertisement_widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops  = array(
			'classname'   => 'widget_728x90_advertisement colornews_custom_widget',
			'description' => __( 'Add your 728x90 Advertisement here', 'colornews' ),
		);
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( $image_link ); ?>"> <?php _e( '728x90 Advertisement Image Link:', 'colornews' ); ?></label>
			...
			<div class="tg-block-wrapper">
				...
				if ( ! empty( $image_url ) ) {
					...
					if ( ! empty( $image_link ) ) {
						...
						$output             .= '<img src="' . $image_url . '" alt="' . $image_alt_text . '" width="728" height="90" rel="nofollow">';
					}
					$output .= '</div>';
					echo $output;
				} ?>

 ...
 * @since ColorNews 1.0
 */
add_action('add_meta_boxes', 'colornews_add_custom_box');

/**
 * Add Meta Boxes.
 */
function colornews_add_custom_box() {
   // Adding layout meta box for page
   add_meta_box('page-layout', __('Select Layout', 'colornews'), 'colornews_page_layout', 'page', 'side', 'default');
   // Adding layout meta box for
   add_meta_box('page-layout', __('Select Layout', 'colornews'), 'colornews_page_layout', 'post', 'side', 'default');
}
...
add_action('pre_post_update', 'colornews_save_custom_meta');

/**
 * save the custom metabox data
 * @hooked to pre_post_update hook
 */
function colornews_save_custom_meta($post_id) {
   global $page_layout, $post;

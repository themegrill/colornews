...
class ColorNews_Theme_Review_Notice {
	...
	public function review_notice() {
		...
		if ( ! get_option( 'colornews_theme_installed_time' ) ) {
	...
	public function review_notice_markup() {
		...
		$dismiss_url              = wp_nonce_url(
			add_query_arg( 'nag_colornews_ignore_theme_review_notice', 0 ),
			'nag_colornews_ignore_theme_review_notice_nonce',
			'_colornews_ignore_theme_review_notice_nonce'
		);
		$temporary_dismiss_url    = wp_nonce_url(
			add_query_arg( 'nag_colornews_ignore_theme_review_notice_partially', 0 ),
			'nag_colornews_ignore_theme_review_notice_partially_nonce',
			'_colornews_ignore_theme_review_notice_nonce'
		);
		 ...
		 */
		if ( ( get_option( 'colornews_theme_installed_time' ) > strtotime( '-14 day' ) ) || ( $ignored_notice_partially > strtotime( '-14 day' ) ) || ( $ignored_notice ) ) {
			return;
		}
		?>
		<div class="notice notice-success colornews-notice theme-review-notice" style="position:relative;">
			<div class="colornews-message__content">
				<div class="colornews-message__image">
					<img class="colornews-logo--png" src="<?php echo esc_url( get_template_directory_uri() . '/inc/admin/images/colornews-square-logo.png' ); ?>" alt="<?php esc_attr_e( 'ColorNews', 'colornews' ); ?>" />
				</div>
				...
				<div class="colornews-message__text">
					...
					<p>(
						...
						printf(
							...
							esc_html__( 'The above word is just to draw your attention. %s', 'colornews' ),
					...
					<p>
						<?php
							printf(
								...
								esc_html__( 'Hope you are having a nice experience with %1$s %2$s %3$s theme. Please provide this theme a nice review.', 'colornews' ),
								'<strong>',
					...
					<strong>
						<?php esc_html_e( 'What benefit would you have?', 'colornews' ); ?>
					</strong>
					<p>
						<?php
							printf(
								/* translators: %s: Smiley icon */
								esc_html__( 'Basically, it would encourage us to release updates regularly with new features & bug fixes so that you can keep on using the theme without any issues and also to provide free support like we have been doing. %s', 'colornews' ),
								'<span class="dashicons dashicons-smiley smile-icon"></span>'
							);
						?>
					</p>
					...
					<div class="links">
						<a href="https://wordpress.org/support/theme/colornews/reviews/?filter=5#new-post" class="btn button-primary" target="_blank">
							...
							<span><?php esc_html_e( 'Sure, I\'d love to!', 'colornews' ); ?></span>
						</a>
						...
						<a href="<?php echo esc_url( 'https://wordpress.org/support/theme/colornews/' ); ?>" class="btn button-secondary" target="_blank">
							<span class="dashicons dashicons-testimonial"></span>
							<span><?php esc_html_e( 'I have a query', 'colornews' ); ?></span>
						</a>
					</div> <!-- /.links -->
				</div> <!-- /.colornews-message__text -->
	...
	public function ignore_theme_review_notice() {
		...
		if ( isset( $_GET['nag_colornews_ignore_theme_review_notice'] ) && isset( $_GET['_colornews_ignore_theme_review_notice_nonce'] ) ) {

			if ( ! wp_verify_nonce( wp_unslash( $_GET['_colornews_ignore_theme_review_notice_nonce'] ), 'nag_colornews_ignore_theme_review_notice_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'colornews' ) );
			}

			if ( '0' === $_GET['nag_colornews_ignore_theme_review_notice'] ) {
				add_user_meta( get_current_user_id(), 'colornews_ignore_theme_review_notice', 'true', true );
			}
		}
	}
	...
	public function ignore_theme_review_notice_partially() {
		...
		/* If user clicks to ignore the notice, add that to their user meta */
		if ( isset( $_GET['nag_colornews_ignore_theme_review_notice_partially'] ) && isset( $_GET['_colornews_ignore_theme_review_notice_nonce'] ) ) {

			if ( ! wp_verify_nonce( wp_unslash( $_GET['_colornews_ignore_theme_review_notice_nonce'] ), 'nag_colornews_ignore_theme_review_notice_partially_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'colornews' ) );
			}

			if ( '0' === $_GET['nag_colornews_ignore_theme_review_notice_partially'] ) {
				update_user_meta( get_current_user_id(), 'nag_colornews_ignore_theme_review_notice_partially', time() );
			}
		}
	}
